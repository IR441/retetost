...
void D3D12HelloTriangle::OnInit()
{
	...
	nv_helpers_dx12::CameraManip.setLookat(glm::vec3(10.0f, 10.0f, 10.0f), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
...
void D3D12HelloTriangle::LoadAssets()
{
	...
	// Create the pipeline state, which includes compiling and loading shaders.
	{
		...
		if (FAILED(D3DCompileFromFile(L"shaders.hlsl", nullptr, nullptr, "VSMain", "vs_5_0", compileFlags, 0, &vertexShader, &Error))) {
			MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, (char*)Error->GetBufferPointer(), dataSize, wlocal,
				dataSize + 1);
			MessageBoxW(NULL, wlocal, L"ShaderVS Compile Error", MB_OK);
			if (!Error)Error->Release();
			//return false;
		}
		if (FAILED(D3DCompileFromFile(L"shaders.hlsl", nullptr, nullptr, "PSMain", "ps_5_0", compileFlags, 0, &pixelShader, &Error))) {
		
			MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, (char*)Error->GetBufferPointer(), dataSize, wlocal,
				dataSize + 1);
			MessageBoxW(NULL, wlocal, L"ShaderPS Compile Error", MB_OK);
			if (!Error)Error->Release();
			//return FALSE;
		}
	...
	CDeviceManager::SetcommandList(m_commandList.Get());
	...
	// Create the vertex buffer.
	{
		...
		//material
		//data
		std::vector<FLOAT> material = {1.0f,1.0f,1.0f,1.0f};
		const UINT materialBufferSize= static_cast<UINT>(material.size()) * sizeof(FLOAT);
		// Create the constant buffer for all matrices
		//buffer�쐬
		m_materialBuffer = nv_helpers_dx12::CreateBuffer(
			m_device.Get(), materialBufferSize, D3D12_RESOURCE_FLAG_NONE,
		...
		//object->Init(m_device.Get());
		CDeviceManager::Init(m_device.Get());
		CManager::Init();
	}
...
//prepare end-----------
	...
	if (m_raster)
	{
		...
		//std::vector< ID3D12DescriptorHeap* >l_heaps = { m_constLightHeap.Get() };
		//m_commandList->SetDescriptorHeaps(static_cast<UINT>(l_heaps.size()), l_heaps.data());
		//// set the root descriptor table 0 to the constant buffer descriptor heap
		//m_commandList->SetGraphicsRootDescriptorTable(
		//	0, m_constLightHeap->GetGPUDescriptorHandleForHeapStart());
		...
		//object->DrawCommand(m_commandList.Get());
		CManager::DrawCommand(m_commandList.Get());
	}
	else
	{
...
//Do raytracing--------------
		...
		// structure, as well as the raytracing output(t0)
...
//CopyRaytracingOutputToBackbuffer-------------------------------
		...
		m_commandList->ResourceBarrier(1, &transition);
//CopyRaytracingOutputToBackbuffer end------------------------
//Present----------------------------------
		transition = CD3DX12_RESOURCE_BARRIER::Transition(
...
void D3D12HelloTriangle::CreateTopLevelAS(
	...
	if (!updateOnly)
	{
		...
		//m_topLevelASGeneratoroldsize = instances.size();
...
void D3D12HelloTriangle::CreateAccelerationStructures() {
	...
	CManager::PushBackInstsnce(&m_instances);
	
	CreateTopLevelAS(m_instances);
	CDeviceManager::Setinstances(&m_instances);
...
ComPtr<ID3D12RootSignature> D3D12HelloTriangle::CreateRayGenSignature() {
	...
	//,{1 /*b0*/, 1, 0, D3D12_DESCRIPTOR_RANGE_TYPE_CBV /*Light parameters*/, 3} 
		});
...
ComPtr<ID3D12RootSignature> D3D12HelloTriangle::CreateHitSignature() {
	...
	//rsc.AddRootParameter(D3D12_ROOT_PARAMETER_TYPE_SRV, 2 /*t2*/); // material
	...
	rsc.AddHeapRangesParameter({ 
		...
		,{3 /*b0*/, 1, 0, D3D12_DESCRIPTOR_RANGE_TYPE_CBV /*Light parameters*/, 1}
		});
...
void D3D12HelloTriangle::CreateShaderResourceHeap() {
	...
	//// Describe and create a constant buffer view for the camera
	////D3D12_CONSTANT_BUFFER_VIEW_DESC cbvDesc = {};
	//cbvDesc.BufferLocation = m_LightBuffer->GetGPUVirtualAddress();
	//cbvDesc.SizeInBytes = m_LightBufferSize;
	//m_device->CreateConstantBufferView(&cbvDesc, srvHandle);
...
void D3D12HelloTriangle::CreateShaderBindingTable() {
	...
	//m_sbtHelper.AddHitGroup(L"HitGroup", { (void*)(object->GetVBAdd()),
	//								   (void*)(object->GetIBAdd()) });
	//m_sbtHelper.AddHitGroup(L"ShadowHitGroup", {});
	//object->Draw(&m_sbtHelper);
	CManager::Draw(&m_sbtHelper);
...
void D3D12HelloTriangle::CreatePlaneVB() {
	...
	Vertex planeVertices[] = {
		{{-15.f, -.8f, 015.f}, {1.0f, 1.0f, 1.0f, 1.0f}}, // 0
		{{-15.f, -.8f, -15.f}, {1.0f, 1.0f, 1.0f, 1.0f}}, // 1
		{{015.f, -.8f, 015.f}, {1.0f, 1.0f, 1.0f, 1.0f}}, // 2
		{{015.f, -.8f, 015.f}, {1.0f, 1.0f, 1.0f, 1.0f}}, // 2
		{{-15.f, -.8f, -15.f}, {1.0f, 1.0f, 1.0f, 1.0f}}, // 1
		{{015.f, -.8f, -15.f}, {1.0f, 1.0f, 1.0f, 1.0f}}  // 4
	};
...
void D3D12HelloTriangle::UpdateCameraBuffer() {
	...
	//XMVECTOR At = XMVectorSet(camera_target.x, camera_target.y, camera_target.z, 0.0f); 
	//XMVECTOR Up = XMVectorSet(camera_up.x, camera_up.y, camera_up.z, 0.0f); camera_up;
...
void D3D12HelloTriangle::CreateMengerSpongeVB()
{
	...
	nv_helpers_dx12::GenerateMengerSponge(3, 0.5, vertices, indices);
	{
